...
RUN update-alternatives --install /usr/bin/clang++ clang++ /usr/bin/clang++-10 10 \
    ...
    && update-alternatives --install /usr/bin/llvm-dis llvm-dis /usr/bin/llvm-dis-10 10 \
    && update-alternatives --install /usr/bin/llvm-link llvm-link /usr/bin/llvm-link-10 10 \
    && update-alternatives --install /usr/bin/opt opt /usr/bin/opt-10 10 \
    && update-alternatives --install /usr/bin/llc llc /usr/bin/llc-10 10
...
RUN git clone https://github.com/IITH-Compilers/IR2Vec.git -b v1.1.0 \
...
RUN pip install "ipython>=7.20" "jupyter" "jupyterlab"
EXPOSE 8888
